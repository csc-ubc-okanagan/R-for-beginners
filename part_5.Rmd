...
The `apply` family of functions is very useful for iterating over data structures, such as data frames. In `lapply()`, the l stands for list, suggesting it takes in lists to iterate or loop over. In this case, the funtion takes a list, `variables_to_convert` and passes that list off, one vector at a time to `as.logical()`.
...
```{r}
random_sample_empty <- vector() # create an empty vector called random_sample_multi
...
for (i in 1:length(random_sample)) { # iterate with a for loop starting with the number 1 and ending with the last index point in random sample, calculated here using length()
  random_sample_empty[i] <- random_sample[i] *2 # for each value at index point 1 through to length(random_sample), multiply that value by 2
}

random_sample_empty # print the result to verify
```

R is a high level language, meaning it's interpreted at run time rather than pre-compiled. This generally makes for loops slow, since R takes time to process each individual step; apply a function to the first element, then to the second element etc. There are ways to make for loops faster in R. One way, using the above example, is to pre-populate the list with a set of values, so R knows in advance how much room to set aside in memory.

```{r}
random_sample_known_length <- vector("numeric", length(random_sample)) # create an empty vector called random_sample_multi

for (i in 1:length(random_sample)) { # iterate with a for loop starting with the number 1 and ending with the last index point in random sample, calculated here using length()
  random_sample_known_length[i] <- random_sample[i] *2 # for each value at index point 1 through to length(random_sample), multiply that value by 2
}

random_sample_known_length # print the result to verify
```

### Iterating over multiple vectors

When speeding things up for a single vector, the user can write these things out. However, when you want to iterate not just over the values of a single vector, but multiple vectors, like the columns of a data frame, this becomes onerous. The apply family of functions takes care of all the heavy lifting for us. We don't need to write code that explicitly loops. Instead, we call an apply function that is designed to efficiently hanld the task for us.
